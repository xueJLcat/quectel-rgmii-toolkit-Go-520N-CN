...
| 短信服务 | 收件箱、发送短信(PDU 编码)、短信存储用量可视化(SM / ME 存储与短信中心号码、CMS 错误码释义) |
| 短信转发 | 新短信自动转发到 Webhook(可配请求方式 / 自定义请求头 / 超时 / JSON 载荷模板)与 Server酱 微信推送(SCT 与 Server酱³ SendKey 自动识别端点),两条通道独立开关、均支持一键测试推送 |
| AT 命令 | Web 端直接发送 AT 命令并查看原始返回 |
| 控制台 | Web Shell,复用后台登录会话,进入即开终端;WebGL 渲染(不可用自动回落),内置回滚缓冲搜索(Ctrl+F) |
| 自动化 | 定时任务调度 |
